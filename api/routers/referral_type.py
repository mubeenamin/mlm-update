...
@router.get("/get_all", response_model=ReferralType)
def get_referral_type(user_id: int, referrer_user_id: int, session:db_dependency):
    # Check if the user is a direct referral
    direct_referral = session.exec(
    ...
    indirect_referral = session.exec(
        select(Referral).where(
            Referral.referred_user_id == user_id,
...
async def create_referral_type(referral_type: ReferralType, session: db_dependency):
    session.add(referral_type)
    session.commit()
    session.refresh(referral_type)
