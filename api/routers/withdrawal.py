...
async def create_withdrawal(db: db_dependency, user: user_dependency,  withdrawal: Withdrawal):
    db.add(withdrawal)
    db.commit()
...
async def get_withdrawal_by_id(db: db_dependency, user: user_dependency, withdrawal_id: int):
...
async def update_withdrawal_by_id(db: db_dependency, user: user_dependency, withdrawal_id: int, withdrawal: WithdrawalUpdate):
...
async def delete_withdrawal_by_id(db: db_dependency, user: user_dependency, withdrawal_id: int):
    result = db.exec(select(Withdrawal).where(Withdrawal.id == withdrawal_id)).first()
    db.delete(result)
    db.commit()
